...
int test_read_from_string()
{
    ...
    u8 data_must_be[] = {
        'a', 1,   0x1e, 'i', 4,   0x00, 0x04, 0x00, 0x00, 'd', 2,   0x9a, 0x02, // digit is folded to minimal possible
                                                                                // size
    ...
    assert(vars_parse(test_string, &data_out));
    assert(memcmp(data_out.ptr, (const char *)data_must_be, data_out.size));
...
int test_parse_from_file()
{
    ...
    assert(parse_block_resources_from_file(path, &br));

    LOG_INFO("\t\tblock: %lld, %.*s", br.id, br.vars.size, br.vars.str);
...
void print_registry(block_registry bro)
{
    ...
    for (u32 i = 0; i < br.length; i++)
    {
        ...
        br.data[i].vars.ptr ? LOG_INFO("%.*s\n", br.data[i].vars.size, br.data[i].vars.ptr) : LOG_INFO("(no data)\n");
    }
}
...
int test_parse_folder()
{
    ...
    assert(read_block_registry(TEST_REGISTRY, &b_reg));
