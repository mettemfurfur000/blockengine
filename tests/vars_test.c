...
int test_basic_data_manip()
{
	...
	status &= var_create(&b, 't', sizeof(src)) == SUCCESS;
	status &= var_set_i(&b, 't', src, sizeof(src)) == SUCCESS;
	status &= var_get_i(b, 't', (i64 *)&dest, sizeof(dest)) == SUCCESS;
...
int test_array_data()
{
	...
	status &= var_set_str(&b, 's', (u8 *)cool_string, strlen(cool_string)) == SUCCESS;
	status &= var_get_str(b, 's', (u8 *)not_cool_string, sizeof(not_cool_string)) == SUCCESS;
...
int test_random_data()
{
	...
	for (int i = 0; i < 64; i++)
	{
		...
		status &= var_set_str(&b, 's', (u8 *)rand_src, rand_size) == SUCCESS;
		if (!status)
		{
			printf("failed to set a string\n");
			break;
		}
		status &= var_get_str(b, 's', (u8 *)rand_dest, rand_size) == SUCCESS;
		if (!status)
		{
...
int test_full_test()
{
	...
	for (int i = 0; i < runs; i++)
	{
		...
		status &= var_set_i(&b, 't', number_in, sizeof(number_in)) == SUCCESS;
		// dbg_data_layout(t.data);
		status &= var_get_i(b, 't', (long long *)&number_out, sizeof(number_out)) == SUCCESS;
		// dbg_data_layout(t.data);
