...
#define DECLARE_DEFAULT_FLAG_HANDLER(name, flag)                               \
    ...
    {                                                                          \
        FLAG_CONFIGURE(dest->flags, flag, atoi(data) ? 1 : 0,                  \
                       strcmp(data, clean_token) == 0)                         \
        return SUCCESS;                                                        \
    }
...
#define DECLARE_DEFAULT_INCREMENTOR(name)                                      \
    void block_res_##name##_incrementor(block_resources *dest) { dest->name++; }

#define DECLARE_DEFAULT_STR_VEC_HANDLER(name)                                  \
    u8 block_res_##name##_vec_str_handler(const char *data,                    \
                                          block_resources *dest)               \
    {                                                                          \
        if (strcmp(data, clean_token) == 0)                                    \
        {                                                                      \
            int i;                                                             \
            char *val;                                                         \
            vec_foreach(&dest->name, val, i) { SAFE_FREE(val); }               \
            if (dest->name.data)                                               \
...
u8 block_res_sounds_vec_handler(const char *data, block_resources *dest)
{
    ...
    for (u32 i = 0; i < tmp.length; i++)
    {
        ...
        snprintf(sound_full_path, sizeof(sound_full_path),
                 ...
                 "%s" SEPARATOR_STR FOLDER_REG_SND SEPARATOR_STR "%s",
                 b_reg->name, tmp.data[i]);
...
const static resource_entry_handler res_handlers[] = {
    ...
    {NULL,
     &block_res_automatic_id_handler,
     "automatic_id",
     NOT_REQUIRED,
     {},
     {},
    ...
    // and locked type
    {NULL,
     &block_res_repeat_times_handler,
     "repeat_times",
     NOT_REQUIRED,
     {},
     {},
    ...
    {NULL,
     &block_res_repeat_increment_vec_str_handler,
     "repeat_increment",
     NOT_REQUIRED,
     {"repeat_times"},
     {},
    ...
    // frames
    {NULL,
     &block_res_fps_handler,
     "fps",
     NOT_REQUIRED,
     {},
     {},
     {"frame_control"}}, // changes frame of a block N times per second. Not a
                         // float!
    {NULL,
     &block_res_anim_controller_handler,
     "frame_controller",
     NOT_REQUIRED,
     {"vars"},
     {},
     {"frame_control"}}, // directly controls the frame of a texture, so it cant
                         // used with fps
    {NULL,
     &block_res_position_based_type_handler,
     "random_frame",
     NOT_REQUIRED,
     {},
     {},
     {"frame_control"}}, // randomizes frame of a block based on its location
    {&block_res_override_frame_incrementor,
     ...
     {},
     {},
     {"frame_control"}}, // directly sets its type from block resources - solid
                         // as rock
    // flips, untested
    {NULL,
     &block_res_flip_controller_handler,
     "flip_controller",
     NOT_REQUIRED,
     {"vars"},
     {},
    ...
    // untested
    {NULL,
     &block_res_rotation_controller_handler,
     "rotation_controller",
     NOT_REQUIRED,
     {"vars"},
     {},
    ...
    // block type things
    {NULL,
     &block_res_type_controller_handler,
     "type_controller",
     NOT_REQUIRED,
     {"vars"},
     {},
     {"type_control"}}, // reads its type from a block data - can be controlled
                        // with scripts
    {NULL,
     &block_res_ignore_type_handler,
     "ignore_type",
     NOT_REQUIRED,
     {},
     {},
     {"type_control"}}, // ignores type and treats each 16x16 square on a
                        // texture as a frame
    // block logic zone
    ...
    // block inputs
    {NULL,
     &block_res_input_names_vec_str_handler,
     "inputs",
     NOT_REQUIRED,
     {"script"},
     {},
...
const char *check_incompat(resource_entry_handler *handler,
                           vec_str_t *seen_entries)
{
    for (u32 j = 0; j < sizeof(handler->incompat) / sizeof(void *); j++)
    {
        ...
        if (seen_entries->length ==
            0) // this entry is first and cant be incompatible with cosmic void
            return NULL;
...
const char *check_slot(resource_entry_handler *handler, vec_str_t *seen_entries,
...
u32 parse_block_resources_from_file(const char *file_path,
                                    block_resources *dest)
{
    ...
    for (u32 i = 0; i < TOTAL_HANDLERS; i++)
    {
        ...
        if (!entry.ptr)
        {
            if (h.is_critical)
            {
                LOG_ERROR("\"%s\" : critical entry \"%s\" is not present",
                          file_path, h.name);
                status = FAIL;
                break;
            }
        ...
        if (dep != NULL)
        {
            LOG_ERROR("\"%s\": \"%s\" is dependent on \"%s\"", file_path,
                      h.name, dep);
            return FAIL;
        }
        ...
        if (inc != NULL)
        {
            LOG_ERROR("\"%s\": \"%s\" is incompatible with \"%s\"", file_path,
                      h.name, inc);
            return FAIL;
        }
        ...
        if (slot != NULL)
        {
            LOG_ERROR("\"%s\": \"%s\" cant be an \"%s\", slot is already taken",
                      file_path, h.name, slot);
            return FAIL;
        }
    ...
    for (u32 j = 0; j < TOTAL_HANDLERS; j++)
        if (res_handlers[j].increment_fn) // if no increment function is
            ...
            for (u32 i = 0; i < br_ref->repeat_increment.length;
                ...
                if (strcmp(res_handlers[j].name,
                           br_ref->repeat_increment.data[i]) == 0)
...
u32 registry_read_block(block_registry *reg_ref, const char *file_path)
{
    ...
    {
        LOG_INFO("Found duplicate resource: %s, resource is not pushed!",
                 file_path);
        return FAIL;
    }
    ...
    while ((entry = readdir(directory)) != NULL)
    {
        if (strcmp(entry->d_name, ".") == 0 || strcmp(entry->d_name, "..") == 0)
            continue;
                 ...
                 DT_REG) // each file will be attempted to be parsed as a block
        {
            snprintf(subpath, sizeof(subpath), "%s" SEPARATOR_STR "%s",
                     folder_path, entry->d_name);

            if (registry_read_block(reg_ref, subpath) != SUCCESS)
            {
                LOG_ERROR("Failed to parse block resources from file: %s",
                          subpath);
                closedir(directory);
                return FAIL;
            }
                 ...
                 DT_DIR) // each folder will be attempted to be parsed as a
                         // folder, full of blocks or other folders
        {
            ...
            {
                LOG_ERROR("Failed to parse block resources from folder: %s",
                          subpath);
                closedir(directory);
                return FAIL;
            }
