...
void create_framebuffer_object(u16 screenWidth, u16 screenHeight, u32 *fbo_ref,
                               u32 *tex_ref)
{
...
int block_renderer_init(int screenWidth, int screenHeight)
{
    ...
    renderer.projectionLoc =
        glGetUniformLocation(renderer.shaderDefault, "uProjection");
    renderer.atlasResizeFactor =
        glGetUniformLocation(renderer.shaderDefault, "uResizeRatio");
    renderer.blockWidthLoc =
        glGetUniformLocation(renderer.shaderDefault, "uBlockWidth");
    renderer.textureLoc =
        glGetUniformLocation(renderer.shaderDefault, "uTexture");
    ...
    create_framebuffer_object(screenWidth, screenHeight, &renderer.fbo,
                              &renderer.fbo_texture);
                              ...
                              u8 local_block_width)
{
    ...
    // Update uniforms
    glUniform2f(renderer.atlasResizeFactor,
                (float)atlas_img->width / g_block_width,
                           ...
                           u8 type, u8 ignore_type, u8 flip)
{
    ...
    renderer.instanceData[idx + 2] =
        (float)(info.atlas_offset_x + actual_frame);
