...
static const float post_vertices[] = {
    -1.0f, -1.0f, 0.0f, 0.0f,
     1.0f, -1.0f, 1.0f, 0.0f,
     1.0f,  1.0f, 1.0f, 1.0f,
    -1.0f,  1.0f, 0.0f, 1.0f
...
void renderer_v2_shutdown(void)
{
    ...
    if (renderer_v2.standard.vao) glDeleteVertexArrays(1, &renderer_v2.standard.vao);
    if (renderer_v2.standard.vbo) glDeleteBuffers(1, &renderer_v2.standard.vbo);
    if (renderer_v2.standard.ebo) glDeleteBuffers(1, &renderer_v2.standard.ebo);
    if (renderer_v2.standard.instance_vbo) glDeleteBuffers(1, &renderer_v2.standard.instance_vbo);
    cleanup_shader_program(&renderer_v2.standard);

    if (renderer_v2.post.vao) glDeleteVertexArrays(1, &renderer_v2.post.vao);
    if (renderer_v2.post.vbo) glDeleteBuffers(1, &renderer_v2.post.vbo);
    if (renderer_v2.post.ebo) glDeleteBuffers(1, &renderer_v2.post.ebo);
...
int renderer_v2_add_instance(float x, float y, u8 frame, u8 type, u8 flags, float scale_x, float scale_y, float rotation)
{
    if (renderer_v2.batch.count >= renderer_v2.batch.capacity)
    {
        u32 new_capacity = renderer_v2.batch.capacity * 2;
        instance_data *new_data = (instance_data *)realloc(renderer_v2.batch.data, new_capacity * sizeof(instance_data));
        if (!new_data)
        {
...
void renderer_v2_end_batch(void)
{
    ...
    glUniform2f(prog->resize_ratio_loc,
                (float)batch->atlas_img->width / g_block_width,
