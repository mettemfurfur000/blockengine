...
typedef struct trait_blob_entry
{
	char *name;
	unsigned char *blob;
	u32 blob_size;
...
typedef vec_t(trait_blob_entry) trait_blob_t;
...
// recompute vars_offsets from the current vars_sample (call after traits
// mutate vars at load time)
void rebuild_vars_offsets(block_resources *res);

// returns a pointer to the compiled trait blob entry for the given name,
// or NULL if the trait was not compiled into this registry.
